...
function TableObjectRenderer(props) {
    ...
    return (
        <>
            <Button variant="unstyled" cursor={"pointer"} justifyContent="flex-start" onClick = {onOpen}>
            ...
            <ModalContent>
                ...
                {(props.display !== "accordion" || props.display !== "table") && finalObject.map((item, index) => typeof item[1] === 'string' && <Flex key={index} py={3} borderTopWidth={1}>
                  ...
                  <Text px={2} py={1}>{item[1].includes('https://') ? <Button as="a" href={item[1]} target="_blank">Download Logo</Button> : item[1]}</Text>
                </Flex>)}
